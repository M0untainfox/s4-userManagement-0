<?php
	...
	class BaseDB
	{
		...
		private function connexionBDD()
		{
			...
			$this->db = new PDO('mysql:host=localhost;dbname=phalcon td0', 'root', '');
				if (DEBUG_PDO) {
	...
	class Database extends BaseDB
	{
		...
		function afficheRole($id)
		{
			$sql = "SELECT count(*) as 'val' FROM role join user on role.id=user.idrole WHERE role.id=$id";
			return $this->executerRequeteAvecResultat($sql);
		}

		function tailleRole()
		{
			$sql = "SELECT count(*) as 'val' FROM role";
			return $this->executerRequeteAvecResultat($sql);
		}
